function [primer_sequences ] = design_primers( sequence, min_Tm, NUM_PRIMERS, misprime_mode, MAX_LENGTH, MIN_LENGTH, tag, is_server )
% DESIGN_PRIMERS
%
...
%   or
%
% [primer_sequences ] = design_primers( sequence, min_Tm, NUM_PRIMERS, misprime_mode,  MAX_LENGTH, MIN_LENGTH, tag );
%
%
...
if exist( 'min_Tm','var' ) && ~isnumeric( min_Tm );
    tag = min_Tm; clear min_Tm;
end;
if ~exist( 'min_Tm','var' ) || isempty(min_Tm); min_Tm = 60; end;
if exist( 'MAX_LENGTH','var' ) && ~isnumeric( MAX_LENGTH );
    tag = MAX_LENGTH; clear MAX_LENGTH;
end;
if ~exist('MAX_LENGTH','var') || isempty(MAX_LENGTH); MAX_LENGTH = 60; end;
if ~exist('MIN_LENGTH','var') || isempty(MIN_LENGTH); MIN_LENGTH = 15; end;
if  ~exist( 'NUM_PRIMERS','var' ) || isempty( NUM_PRIMERS) || NUM_PRIMERS == -1;  NUM_PRIMERS = 0; end;
if ~exist( 'misprime_mode','var' ) || isempty(misprime_mode); misprime_mode = 0; end;
if ~exist('is_server','var') || isempty(is_server); is_server = 0; end;
...
for p = MIN_LENGTH:MAX_LENGTH  % STOP[forward](1)
    ...
    for q = q_min:q_max
        ...
        if ( Tm_precalculated( q, p ) > min_Tm);
            ...
            if (NEW_SCORE);
                scores_stop( p,q,1) = scores_stop( p,q,1) + misprime_score_weight * ( misprime_score_forward(p) +  misprime_score_reverse(q) );
            end;
        end;
    end;
end;
        ...
        for q = q_min:q_max % STOP[reverse]
            ...
            if ( scores_stop(p,q,n) < MAX_SCORE ) % previous primer ends had overlap with good Tm and were scored
                ...
                if last_primer_length <= MAX_LENGTH && last_primer_length >= MIN_LENGTH
                    ...
                    if (NEW_SCORE);
                        scores_final( p,q,n) = scores_final( p,q,n) + misprime_score_weight * ( misprime_score_forward(p) +  misprime_score_reverse(q) );
                    end;
                end;
            end;
        end;
                ...
                for p = min_p:max_p
                    ...
                    for q = min_q:max_q
                        ...
                        if ( Tm_precalculated( q, p ) ) > min_Tm
                            ...
                            if (NEW_SCORE);
                                potential_score = potential_score + misprime_score_weight * ( misprime_score_forward(p) +  misprime_score_reverse(q) );
                            end;
...
if ~exist( 'tag','var' ) || isempty(tag); tag = 'primer'; end;
if ~is_server;
    ...
    for m = 1:size(primers,2)
        ...
        if ( primers(3,m) == 1 ); % forward dir
            ...
            if ( num_match_forward( endpos ) > CUTOFF );
                ...
                for q = 1:length( problem_primers);
                    fprintf( ' %d', problem_primers(q) );
                end;
                fprintf( '\n' );
            end;
        else
    ...
    for m = 1:size(primers,2)
        ...
        if ( primers(3,m) == 1 ); % forward dir
            ...
            if ( num_match_forward( endpos ) > CUTOFF );
                problem_primers = find_primers_affected( primers, best_match_forward( endpos ) );
                fprintf( '@WARNING: Primer {%d%s%s} can misprime with (%d)-residue overlap to position [%d], which is covered by primers ', m, F_or_R(m), lower(F_or_R(m)), num_match_forward(endpos), best_match_forward( endpos )  );
                for q = 1:length( problem_primers);
                    fprintf( ' {%d%s%s}.', problem_primers(q), F_or_R(problem_primers(q)), lower(F_or_R(problem_primers(q))) );
                end;
                fprintf( '\n' );
            end;
        else
            ...
            if ( num_match_reverse( endpos ) > CUTOFF );
                problem_primers = find_primers_affected( primers, best_match_reverse( endpos ) );
                fprintf( '@WARNING: Primer {%d%s%s} can misprime with (%d)-residue overlap to position [%d], which is covered by primers ', m, F_or_R(m), lower(F_or_R(m)), num_match_reverse(endpos), best_match_reverse( endpos )  );
                for q = 1:length( problem_primers);
                    fprintf( ' {%d%s%s}.', problem_primers(q), F_or_R(problem_primers(q)), lower(F_or_R(problem_primers(q))) );
                end;
                fprintf( '\n' );
